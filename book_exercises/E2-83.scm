...
(define (install-real-package)
	...
	(define (tag x)
		(attach-tag 'real x))
	(put 'make 'real
		(lambda (x) (tag (make-real-number x))))
	(put 'raise '(real)
		(lambda (x) (raise x))))
...
(define (install-scheme-number-package)
	...
	(define (raise x) ((get 'make 'rational) x 1))
...
(define (install-rational-package)
	...
	(define (div-rat x y)
		(make-rat (* (numer x) (denom y))
			(* (denom x) (numer y))))
	(define (equ? x y)
		(and (equal? (numer x) (numer y)) (equal? (numer x) (denom y))))
	(define (=zero? x)
		(equal? (numer x) 0))
	(define (raise x)
...
(define (install-complex-package)
	...
	(define (equ? z1 z2)
		(equal? (real-part z1) (real-part z2)))
	(define (=zero? z1)
		(equal? (real-part z1) 0))
	;; internal procedures
