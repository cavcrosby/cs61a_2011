...
(define (eval-1 exp)
  (cond ((constant? exp) exp)
    ...
    ((let-exp? exp) 
	 (eval-1 (apply-1 
				(list 'lambda 
					(map-1 car (cadr exp))  ; get the vars lol, then get the let's vars symbols ((x (+ 2 3)) (y (+ 1 0))) getting x y
					(caddr exp)) ; this should be the let's body
				(map-1 eval-1 
					(map-1 cadr (cadr exp)))))) ; get the vars lol, getting the argument expressions, then evaulate each of them
	((lambda-exp? exp) exp)
...
(define (apply-1 proc args)
  (cond ((PROCEDURE? proc)	; use underlying Scheme's APPLY
	 (apply PROC args))
	((lambda-exp? proc)
	 (eval-1 (substitute (caddr proc) ; the body parameter
...
(define (map-1 fn args)
			(if (null? args)
				'()
				(cons (apply-1 fn (list (car args))) (map-1 fn (cdr args))))) ; apply-l is needed to make the function work, mainly to substitute values and evaluate
